...
// end include: /tmp/tmpcxakxwhz.js
// include: /tmp/tmpqwcdjq08.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpqwcdjq08.js
// include: /tmp/tmp15748n6z.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach(function(task) {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmp15748n6z.js
...
var ASM_CONSTS = {
  131431: () => { if (document.fullscreenElement) return 1; },  
 131477: () => { return document.getElementById('canvas').width; },  
 131529: () => { return parseInt(document.getElementById('canvas').style.width); },  
 131597: () => { document.exitFullscreen(); },  
 131624: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 131697: () => { if (document.fullscreenElement) return 1; },  
 131743: () => { return document.getElementById('canvas').width; },  
 131795: () => { return screen.width; },  
 131820: () => { document.exitFullscreen(); },  
 131847: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 131980: () => { return window.innerWidth; },  
 132006: () => { return window.innerHeight; },  
 132033: () => { if (document.fullscreenElement) return 1; },  
 132079: () => { return document.getElementById('canvas').width; },  
 132131: () => { return parseInt(document.getElementById('canvas').style.width); },  
 132199: () => { if (document.fullscreenElement) return 1; },  
 132245: () => { return document.getElementById('canvas').width; },  
 132297: () => { return screen.width; },  
 132322: () => { return window.innerWidth; },  
 132348: () => { return window.innerHeight; },  
 132375: () => { if (document.fullscreenElement) return 1; },  
 132421: () => { return document.getElementById('canvas').width; },  
 132473: () => { return screen.width; },  
 132498: () => { document.exitFullscreen(); },  
 132525: () => { if (document.fullscreenElement) return 1; },  
 132571: () => { return document.getElementById('canvas').width; },  
 132623: () => { return parseInt(document.getElementById('canvas').style.width); },  
 132691: () => { document.exitFullscreen(); },  
 132718: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 132776: () => { return screen.width; },  
 132801: () => { return screen.height; },  
 132827: () => { return window.screenX; },  
 132854: () => { return window.screenY; },  
 132881: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 132934: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 133005: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 133062: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 133318: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 133389: () => { if (document.fullscreenElement) return 1; },  
 133435: () => { return window.innerWidth; },  
 133461: () => { return window.innerHeight; },  
 133488: () => { if (document.pointerLockElement) return 1; },  
 133535: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 135693: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 135857: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 135961: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 138960: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 139026: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 139419: ($0) => { miniaudio.untrack_device_by_index($0); },  
 139462: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 139587: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
